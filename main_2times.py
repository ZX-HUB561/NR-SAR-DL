...
import scipy
import numpy as np
from model_yang import siamese_net
...
def rsmain(args):
    ...
    loss1 = tf.reduce_mean(tf.abs(y1 - x2))
    loss2 = tf.reduce_mean(tf.abs(y2 - x1))
    ...
    mat_im1 = scipy.io.loadmat(r"Z:\data_pool\TerraSAR\ruhr\iq1.mat")
    im1_val = mat_im1['iq1']/1
    mat_im2 = scipy.io.loadmat(r"Z:\data_pool\TerraSAR\ruhr\iq2.mat")
    im2_val = mat_im2['iq2']/1
    mat_ss = scipy.io.loadmat("Z:/data_pool/paper2exp/similarity.mat")
    ss12_val = mat_ss['similarity']
    im1_val_nor = im1_val/1
    im2_val_nor = im2_val/1
